...
func networkWithBuyOrderBookObjects(t *testing.T, n int) (*network.Network, []*types.BuyOrderBook) {
	...
	for i := 0; i < n; i++ {
		state.BuyOrderBookList = append(state.BuyOrderBookList, &types.BuyOrderBook{Creator: "ANY", Index: strconv.Itoa(i)})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.BuyOrderBook)
			}
		})
	}
}
...
func TestListBuyOrderBook(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.BuyOrderBook[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.BuyOrderBook)
	})
}
