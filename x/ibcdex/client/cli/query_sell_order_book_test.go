...
func networkWithSellOrderBookObjects(t *testing.T, n int) (*network.Network, []*types.SellOrderBook) {
	...
	for i := 0; i < n; i++ {
		state.SellOrderBookList = append(state.SellOrderBookList, &types.SellOrderBook{Creator: "ANY", Index: strconv.Itoa(i)})
	}
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.SellOrderBook[j-i])
			}
		}
	})
